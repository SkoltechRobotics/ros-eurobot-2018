...
class ObstaclesPublisher():
    def __init__(self):
        ...
        self.cubes = self.heaps2cubes(self.heaps)
        ...
        rospy.Timer(rospy.Duration(0.2), self.pub_timer_callback)

    def heaps2cubes(self, heaps):
        cubes = []
        for i in range(len(heaps)):
            for j in range(self.n_cubes_in_heap):
                ...
                cube.polygon.points = [Point32(*heaps[i][j])]
                cubes.append(cube)
        return cubes
    ...
    def pub_timer_callback(self, event):
        ...
        obstacle_msg.obstacles = self.cubes
